...
const HFAT_SERVER_PORT = 18000
const TEST_PATH_AND_QUERY = "/whateverPath?TEST=98&sha=27892572805702572"
const TEST_SERVER_1_STATUS_CODE = 200
const TEST_SERVER_2_STATUS_CODE = 299

func TestTimeConsuming(t *testing.T) {

	requestSnifferChannel1 := make(chan *http.Request)
	requestSnifferChannel2 := make(chan *http.Request)
	go StartTestHttpServer(TEST_SERVER_1_PORT, TEST_SERVER_1_RESPONSE_BODY, TEST_SERVER_1_STATUS_CODE, requestSnifferChannel1)
	go StartTestHttpServer(TEST_SERVER_2_PORT, TEST_SERVER_2_RESPONSE_BODY, TEST_SERVER_2_STATUS_CODE, requestSnifferChannel2)
	...
	if response.StatusCode != TEST_SERVER_2_STATUS_CODE {
		t.Error("Did expect status code %v, but not: %v\n", TEST_SERVER_2_STATUS_CODE, response.StatusCode)
	}
